...
const getSpecies = data => { 
     return data.map(animals => animals.species);
}
const speciesList = getSpecies(data);
console.log(speciesList);

  /*     2.- Necesitamos una lista con aquellos personajes que sean Humanos. */

const filterBySpecies = data => {
    return data.filter(species => species.species === "Alien")
}

const test2 = filterBySpecies(data)
console.log(test2)


    //   3.- Necesitamos poder obtener la información de un personaje específico con base en su id
    
const getCharacterByID = (data, id) => {
    const character = data.find(item => item.id === id) 
    return character ? {...character} : `Character with ID not found.`
}

const test3 = getCharacterByID(data, 7)
console.log(test3)
...
const getMatchingNames = (characters, searchString) => {
    return characters 
        .filter(character => character.name.includes(searchString)) 
        .map(character => character.name)
}

const test4 = getMatchingNames(data, "Ri")
console.log(test4)

    //   5.- Necesito saber la cantidad de personajes cuyo género sea "Male"

const filterByGender = characters => {
    return characters.filter(character => character.gender === "Male").length
}

const test5 = filterByGender(data)
console.log(test5)

//   6.- Necesito una nueva lista que contenga sólo la información de nombre, tipo y especie de cada personaje

const extractCharacterInfo = characters => {
    return characters.map(({ name, type, species }) => ({ name, type, species }))
}

const test6 = extractCharacterInfo(data)
console.log(test6)